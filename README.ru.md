...
Необязательное поле `peer_list_url` — P2P-трекер этого репозитория. Если его
нет, пакеты репозитория качаются только с его зеркал, а об установках никуда
не сообщается. Вшитый трекер `keytron-prime.org/warp` используется только
для встроенного `k1os`.
